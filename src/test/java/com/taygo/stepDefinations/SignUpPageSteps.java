...
public class SignUpPageSteps extends DriverFactory {
    ...
    public void userVerifyErrorMessagesForFirstNameLastNameEmailAndMobileNum(String fn, String ln, String email, String mobilnum) {
        ...
        if (!fn.isBlank ()) {
            Assert.assertEquals(
                    fn,
                    signUpPage.fnErrorText.getText());
        }
        if (!ln.isBlank()) {
            Assert.assertEquals(
                    ln,
                    signUpPage.lnErrorText.getText());
        }
        if (!email.isBlank()) {
            Assert.assertEquals(
                    email,
                    signUpPage.emailErrorText.getText());
        }
        if (!mobilnum.isBlank()) {
            Assert.assertEquals(
                    mobilnum,
                    signUpPage.mobileErrorText.getText());
        }
    }
